...
var Magix={
    ...
    **/
     ...
     * @param {Boolean} cfg.originalHREF 是否使用原始的location.href,默认会对location.href解码
     * @example
     * Magix.start({
     *      useHistoryState:true,
...
var Router=MIX({
    ...
    navigate:function(params,pathname){
        if(Magix.isObject(params)){
            var arr=[];
            var cfg=Magix.config();
            var oHREF=cfg.originalHREF;
            for(var p in params){
                arr.push(p+'='+(oHREF?params[p]:ENCODE(params[p])));
            }
            params=arr.join('&');
            if(pathname){
